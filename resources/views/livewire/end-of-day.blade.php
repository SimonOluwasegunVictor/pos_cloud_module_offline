<div class="min-h-screen {{ $darkMode ? 'bg-gray-900' : 'bg-gray-50' }} transition-colors duration-200">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div class="{{ $darkMode ? 'bg-gray-800' : 'bg-white' }} rounded-xl shadow-sm p-6 transition-colors duration-200">
            <h2 class="text-2xl font-bold {{ $darkMode ? 'text-white' : 'text-gray-900' }} mb-6">End of Day Report</h2>

            <div class="text-center py-12">
                <svg class="mx-auto h-12 w-12 {{ $darkMode ? 'text-gray-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                <h3 class="mt-2 text-sm font-medium {{ $darkMode ? 'text-gray-300' : 'text-gray-900' }}">No report available</h3>
                <p class="mt-1 text-sm {{ $darkMode ? 'text-gray-500' : 'text-gray-500' }}">End of day reports will be generated here.</p>
            </div>
        </div>
    </div>
</div>
